<?php
	...
	*/
	...
	/* Стилизация кнопок количества товаров
	add_action( 'woocommerce_before_quantity_input_field', 'truemisha_quantity_plus', 10 );
	add_action( 'woocommerce_after_quantity_input_field', 'truemisha_quantity_minus', 10 );
	 
	function truemisha_quantity_plus() {
		echo '<button class="btn btn-sm btn-corporate-1" type="button" style="width: 30px;" onclick="this.nextElementSibling.stepDown(); this.nextElementSibling.onchange();">+</button>';
	}
	 
	function truemisha_quantity_minus() {
		echo '<button class="btn btn-sm btn-corporate-1" type="button" style="width: 30px;" onclick="this.previousElementSibling.stepUp(); this.previousElementSibling.onchange();">-</button>';
	} */
	...
	/* Убираем теги из описания категории товара */
	remove_action( 'woocommerce_archive_description', 'woocommerce_taxonomy_archive_description', 10 );
	add_action( 'woocommerce_archive_description', 'new_woocommerce_archive_description', 10 );
	function new_woocommerce_archive_description() {
		echo strip_tags( term_description() );
	}
	
	
	/* Добавляем описание категории на странице категорий */
	function echo_cat() {
	...
	/* Изменям название страниц для категорий */
	...
	function wp_kama_woocommerce_page_title_filter( $page_title ){
		...
		switch ( $page_title ) {
			case 'Фанера':
			$page_title = 'Купить фанеру в Москве';
			break;
			
			case 'Фанера ФСФ':
			$page_title = 'Купить фанеру ФСФ в Москве';
			break;
		}
	...
	function choose_background() {
		if ( is_product_category() ) {
			...
			switch ( $cat_name ) {
				default;
					echo 'style="background-image: url('.get_template_directory_uri().'/img/header-bg-2.jpg);"';
					break;
				case 'Кромка':
					echo 'style="background-image: url('.get_template_directory_uri().'/img/kromka-bg.jpg);"';
					break;
				case 'Столешницы':
					echo 'style="background-image: url('.get_template_directory_uri().'/img/stoleshnya-bg.jpg);"';
					break;
				case 'Фанера':
					echo 'style="background-image: url('.get_template_directory_uri().'/img/fanera-bg.jpg);"';
					break;
				case 'Фасады':
					echo 'style="background-image: url('.get_template_directory_uri().'/img/fasady-bg.jpg);"';
					break;
				case 'МДФ':
					echo 'style="background-image: url('.get_template_directory_uri().'/img/mdf-bg.jpg);"';
					break;
				case 'ДСП':
					echo 'style="background-image: url('.get_template_directory_uri().'/img/dsp-bg.jpg);"';
					break;
				case 'ЛМДФ':
					echo 'style="background-image: url('.get_template_directory_uri().'/img/lmdf-bg.jpg);"';
					break;
			}
		} else {
			echo 'style="background-image: url('.get_template_directory_uri().'/img/header-bg-2.jpg);"';
